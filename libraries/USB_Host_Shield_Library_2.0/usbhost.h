...
uint16_t MAX3421e< SPI_SS, INTR >::reset() {
        uint16_t i = 0;
        // Serial.println("in usbhost reset");
        regWr(rUSBCTL, bmCHIPRES);
        regWr(rUSBCTL, 0x00);
        while(++i) {
                // Serial.print("in usbhost reset while loop: i=");
                // Serial.println(i);
...
int8_t MAX3421e< SPI_SS, INTR >::Init() {
        // Serial.println("in usbhost Init");
        XMEM_ACQUIRE_SPI();
        // Moved here.
        ...
        if(reset() == 0) { //OSCOKIRQ hasn't asserted in time
                Serial.println("in usbhost Init, reset() == 0, returning -1");
                return ( -1);
        }
